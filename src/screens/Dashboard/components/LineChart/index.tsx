...
export function LineChart({
  ...
  const toX = (time: number) =>
    lastTime === firstTime
      ? 0
      : ((time - firstTime) / (lastTime - firstTime)) * 100;
  const toY = (value: number) => 100 - ((value - min) / (max - min)) * 100;
  const linePath = (points: ChartPoint[]) =>
    points
      .map(
        (point, index) =>
          `${index === 0 ? 'M' : 'L'} ${toX(point.t).toFixed(2)} ${toY(point.value).toFixed(2)}`,
      )
      .join(' ');
  ...
  const hoverEntries = hoverPoint
    ? series.flatMap((item, index) => {
        const point = hoverIndex !== null ? item.points[hoverIndex] : undefined;
        return point
          ? [{ id: item.id, label: item.label, value: point.value, index }]
          : [];
      })
    : [];
  ...
  return (
    <div className={cn('flex flex-col gap-2', className)}>
      ...
      <div className='flex justify-between pl-12 text-xs text-muted'>
        <span>{formatDate(firstTime)}</span>
        <span className='hidden sm:inline'>
          {formatDate((firstTime + lastTime) / 2)}
        </span>
